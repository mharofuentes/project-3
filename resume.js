<!DOCTYPE html>
<html lang="en">
<head>
<style>

	body {background-color: LavenderBlush;}
		h1   {
		color: Navy;
		font-family: Ariel;
		font-size: 210%}
		h2    {                       
        		color: Hotpink;
				font-family: Ariel;}
		h3    {
				color: RebeccaPurple;}
		p    {
		color: Indigo;
		border: 3px Lightcoral;} 
		
		li    { 
				color: DarkCyan;}
		</style>
  
	 <meta charset="UTF-8">
    <title>Resume</title>
	<script type="text/javascript">
	</script>
	</head>
      <nav>
        <ul>
          <li><a href="index.html">Homepage</a></li>
          <li><a href="resume.html">Resume</a></li>
          <li><a href="page3.html">Hamster Species</a></li>
	   </ul>
      </nav>
    <h1>Mitzi Haro Resume</h1>
      <h1>Contact Information</h1>
      <p>(773)609-3007</p>
      <p>mitziharo@gmail.com</p>
      <p></p>
      <h2>Education</h2>
	<ul>
        <li>Highschool: UCSN Hector P. Garcia M.D.</li>
        <li>Bachelors: Information Technology and Management</li>
		<li>Illinois Institute of Technology (expected graduation: Spring 2023)</li>
		<li> Related course work: Java,Python,SQL,Project Management, Network Admin, Enterprise End-User Sys Admin 
		<!--The div element for the map -->
		<div id="map"></div>
		
	</ul>
      <h2>Work Experience</h2>
      <h3>Book Seller</h3>
	  <p></p>
	<ul>
	  <li>IIT Barnes and Noble Bookstore, Chicago,IL </li>
	  <li> January 2020-Present </li>
	  <li>Organize and sell Textbooks</li>
	  </ul>
	  <p></p>
	  <h3>Galvin Libary Circulation Student Assistant</h3>
	  <ul>
	  <li>IIT Galvin Library, Chicago,IL </li>
	  <li> Assist at main front desk, organize books. </li>
	  </ul>
	  <p></p>
	  <h3>STEM Instructor</h3>
	  <ul>
	  <li>Arturo Velazquez Institute, Chicago,IL</li> 
	  <li>July 2019 - December 2021</li>
	  <li>Taught civil engineering to kids age 7-14</li>
	  </ul>
	  <p></p>
	  <h3>Camp Counselor and Tutor </h3>
	  <ul>
	  <li> City Colleges of Chicago, Chicago, IL </li>
	  <li> Summer of 2018,2019,2020</li>
	  <li>Tutored and guided middle school age kids </li>
	  </ul>
    <footer>
    </footer>
  </body>
  </script>
</html>
